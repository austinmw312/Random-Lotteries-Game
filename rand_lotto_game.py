...
def generate_lottos():
    ...
    b_valid = False
    while b_valid == False:
        b = input("Max expected value: ")
        if b == "":
            print("Enter a number between 11 and 90.")
            continue
        max_eval = int(b)
        if (11 <= max_eval <= 90):
            b_valid = True
    ...
    x_valid = False
    while x_valid == False:
        x = input("Max difference in expected values: ")
        if x == "":
            print("Please enter a number between 0 and 80 (<5 yields close lotteries).")
            continue
        eval_range = int(x)
        if (0 <= eval_range <= 80):
            x_valid = True
    ...
    y_valid = False
    while y_valid == False:
        y = input("Max difference in outcome uncertainties: ")
        if y == "":
            print("Enter a number between 0 and 2000. <100 yields lotteries with similar uncertainty.")
            continue
        ou_range = int(y)
        if (0 <= ou_range < 2001):
            y_valid = True
    ...
    z_valid = False
    while z_valid == False:
        z = input("Min outcome uncertainty: ")
        if z == "":
            print("Enter a number between 0 and 1200.")
            continue
        min_ou = int(z)
        if (0 <= min_ou <= 1200):
            z_valid = True
    ...
    a_valid = False
    while a_valid == False:
        a = input("Max outcome uncertainty: ")
        if a == "":
            print("Enter a number between 10 and 2000.")
            continue
        max_ou = int(a)
        if (10 <= max_ou <= 2000):
            a_valid = True
    ...
    # ensure outcome uncertainty within desired range
    while (abs(exp_val_v1 - exp_val_v2) > eval_range) or (exp_val_v1 > max_eval) \
        ...
        or (outcome_uncertainty_v1 < min_ou or outcome_uncertainty_v2 < min_ou) \
        or (outcome_uncertainty_v1 > max_ou or outcome_uncertainty_v2 > max_ou):
